...
def _flush_valkey(namespace: str) -> None:
...
def ui_client(cluster_config, keycloak_config):
    """Return (client_id, client_secret) for the ``cost-management-ui`` client."""
...
def flush_cache_before_suite(cluster_config):
    ...
    _flush_valkey(cluster_config.namespace)
...
class TestRestrictedUserAccess:
    """S-06 / S-07: cost-openshift-viewer sees only OCP data; no sources, no cost-models."""

    def test_viewer_cannot_see_sources(
        self, gateway_url: str, viewer_auth: dict, http_session: requests.Session,
    ):
        """Viewer (cost-openshift-viewer) has no integration:read → 0 sources."""
        response = http_session.get(
            f"{gateway_url}/cost-management/v1/sources/",
            headers=viewer_auth,
            timeout=30,
        ...
        assert data["meta"]["count"] == 0, (
            f"Viewer should see 0 sources, got {data['meta']['count']}. "
            "Kessel may not be filtering correctly."
        )
...
class TestAccessBoundaries:
    ...
    def test_source_visibility_differs_by_role(
        ...
        """Admin sees source(s); viewer sees none."""
        ...
        assert viewer_count == 0, (
            f"Viewer should see 0 sources, got {viewer_count}"
        )
...
class TestNoLeakage:
    ...
    def test_viewer_source_detail_returns_404(
        ...
        """If admin can see source ID=X, viewer GETting that ID returns 404."""
        admin_resp = http_session.get(
            f"{gateway_url}/cost-management/v1/sources/",
            headers=admin_auth,
            timeout=30,
        ...
        assert viewer_resp.status_code == 404, (
            f"Viewer should get 404 for source {source_id}, "
...
class TestROSNoAccessDenied:
    ...
    def test_no_access_denied_recommendations(
        ...
        """User with no bindings cannot access ROS recommendations."""
        resp = http_session.get(
            f"{gateway_url}/cost-management/v1/recommendations/openshift",
            headers=no_access_auth,
            timeout=30,
        ...
        assert resp.status_code == 403, (
            f"Expected 403 for user with no bindings, got {resp.status_code}: "
            f"{resp.text[:300]}"
        )
...
class TestThreeWayAccessComparison:
    ...
    def test_source_visibility_gradient(
        ...
        """Admin sees sources, viewer sees none, no-access sees none."""
        results = {}
        for label, auth in [
            ("admin", admin_auth),
            ("viewer", viewer_auth),
        ...
        assert results["viewer"] == 0, "Viewer should see 0 sources"
    ...
    def test_recommendations_access_gradient(
        ...
        """Admin 200, viewer 200, no-access 403 on recommendations."""
        for label, auth, expected in [
            ("admin", admin_auth, 200),
            ("viewer", viewer_auth, 200),
            ("no-access", no_access_auth, 403),
