...
spicedb_write_relationships() {
    local payload="$1"
    echo "$payload" | spicedb_grpcurl -d @ \
        "$(_spicedb_url)" \
...
do_grant() {
    ...
    local updates
    updates=$(build_spicedb_grant_updates "$username" "$role_slug" "$org_id")
    local payload
    payload=$(jq -n --argjson updates "$updates" '{"updates": $updates}')
    spicedb_write_relationships "$payload"
...
do_revoke() {
    ...
    updates=$(cat <<EOF
[
  {"operation":"OPERATION_DELETE","relationship":{"resource":{"objectType":"rbac/role_binding","objectId":"$rb_id"},"relation":"t_granted","subject":{"object":{"objectType":"rbac/role","objectId":"$role_slug"}}}},
  {"operation":"OPERATION_DELETE","relationship":{"resource":{"objectType":"rbac/role_binding","objectId":"$rb_id"},"relation":"t_subject","subject":{"object":{"objectType":"rbac/principal","objectId":"redhat/$username"}}}},
  {"operation":"OPERATION_DELETE","relationship":{"resource":{"objectType":"rbac/workspace","objectId":"$org_id"},"relation":"t_binding","subject":{"object":{"objectType":"rbac/role_binding","objectId":"$rb_id"}}}},
  {"operation":"OPERATION_DELETE","relationship":{"resource":{"objectType":"rbac/tenant","objectId":"$org_id"},"relation":"t_binding","subject":{"object":{"objectType":"rbac/role_binding","objectId":"$rb_id"}}}}
]
EOF
    )
    local payload
...
do_check() {
    ...
    local result
    result=$(spicedb_grpcurl -d '{
        "resource": {"objectType": "rbac/workspace", "objectId": "'"$org_id"'"},
        "permission": "'"$permission"'",
        "subject": {"object": {"objectType": "rbac/principal", "objectId": "redhat/'"$username"'"}}
    }' "$(_spicedb_url)" "authzed.api.v1.PermissionsService/CheckPermission" 2>/dev/null)

    local perm_status
    perm_status=$(echo "$result" | jq -r '.permissionship // "PERMISSIONSHIP_UNSPECIFIED"')

    if [ "$perm_status" = "PERMISSIONSHIP_HAS_PERMISSION" ]; then
...
do_sync() {
    ...
    while IFS= read -r user; do
        ...
        build_spicedb_grant_updates "$username" "$DEFAULT_ROLE" "$org_id" | \
            jq -c '.' > "${tmpfile}.part"
        jq -s '.[0] + .[1]' "$tmpfile" "${tmpfile}.part" > "${tmpfile}.new"
        mv "${tmpfile}.new" "$tmpfile"
        synced=$((synced + 1))
        log_info "  Queued '$username' (org: $org_id)"
    done < <(echo "$users_json" | jq -c '.[]')

    collect_service_account_updates_spicedb > "${tmpfile}.sa"
    local sa_size
    sa_size=$(jq 'length' "${tmpfile}.sa" 2>/dev/null || echo 0)
    if [ "$sa_size" -gt 0 ]; then
        jq -s '.[0] + .[1]' "$tmpfile" "${tmpfile}.sa" > "${tmpfile}.new"
        mv "${tmpfile}.new" "$tmpfile"
    fi

    local update_count
    update_count=$(jq 'length' "$tmpfile")
    ...
    # Deduplicate: SpiceDB rejects duplicate relationships in a single WriteRelationships call
    jq -c 'unique_by(.relationship | tostring)' "$tmpfile" > "${tmpfile}.dedup"
    mv "${tmpfile}.dedup" "$tmpfile"
    update_count=$(jq 'length' "$tmpfile")

    log_info "Writing $update_count relationships to SpiceDB (OPERATION_TOUCH, atomic, deduped)..."
    jq -c '{updates: .}' "$tmpfile" > "${tmpfile}.payload"

    local result
    result=$(cat "${tmpfile}.payload" | spicedb_grpcurl -d @ \
        "$(_spicedb_url)" \
        authzed.api.v1.PermissionsService/WriteRelationships 2>&1)
    if [ $? -eq 0 ]; then
...
collect_service_account_updates_spicedb() {
    ...
    while IFS= read -r client; do
        ...
        build_spicedb_grant_updates "$sa_username" "$DEFAULT_ROLE" "$org_id" | \
            jq -c '.' > "${satmp}.part"
        jq -s '.[0] + .[1]' "$satmp" "${satmp}.part" > "${satmp}.new"
        mv "${satmp}.new" "$satmp"
