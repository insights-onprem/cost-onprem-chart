...
while [ $ATTEMPT -lt $MAX_ATTEMPTS ]; do
    if kubectl get pod "$POD_NAME" -n "$NAMESPACE" >/dev/null 2>&1; then
        ...
        if [ "$POD_STATUS" = "Running" ]; then
            # Test database connectivity (use koku user - the superuser for this image)
            if kubectl exec "$POD_NAME" -n "$NAMESPACE" -- psql -U koku -d koku -c "SELECT 1" >/dev/null 2>&1; then
...
# The password is already set during container initialization from the secret
log_info "Database user $KOKU_DB_USER is already configured by PostgreSQL initialization"
...
if kubectl exec "$POD_NAME" -n "$NAMESPACE" -- psql -U "$KOKU_DB_USER" -d "$KOKU_DB_NAME" -c "SELECT COUNT(*) FROM django_migrations;" >/dev/null 2>&1; then
    MIGRATION_COUNT=$(kubectl exec "$POD_NAME" -n "$NAMESPACE" -- psql -U "$KOKU_DB_USER" -d "$KOKU_DB_NAME" -tAc "SELECT COUNT(*) FROM django_migrations;")
